...
-- P.10-C6.SEO-IMPL-R2 — rollback exato da persistência SEO (repository-only)
-- NÃO executar sem validação e autorização explícita.

begin;

-- Remove a assinatura R2 e restaura exatamente o contrato anterior da RPC.
...
-- Restaura o guard da Fase 3-U.1, mantendo a proteção de concorrência de tags
-- e removendo apenas os três flags SEO da definição de mudança material.
...
alter table public.blog_posts
  drop constraint blog_posts_seo_sitemap_requires_indexing_check;

alter table public.blog_posts
  drop column seo_include_in_sitemap,
  drop column seo_allow_following,
  drop column seo_allow_indexing;

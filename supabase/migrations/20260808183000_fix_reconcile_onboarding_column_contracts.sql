-- Migration: 20260808183000_fix_reconcile_onboarding_column_contracts.sql
-- Description: Corrects invalid column references (updated_at) in user_company_access and categorias_contas_pagar.
...
BEGIN
    ...
    -- B. Provisionar Regras de Roteamento Padrão
    -- Somente se houver conta Mercado Pago vinculada (simulado para o onboarding)
    -- As regras reais dependem de bank_accounts.id, que é provisionado pelo usuário depois.
    -- Aqui apenas garantimos que a estrutura de tabelas está pronta.
END;
$function$;

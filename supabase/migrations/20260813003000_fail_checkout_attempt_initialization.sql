-- PROTOCOLO: VEJAMAIS_STRIPE_CREATING_ATTEMPT_ATOMIC_RECOVERY_ACTUAL_IMPLEMENTATION
-- OBJETIVO: Implementar RPC de compensação para falhas locais pré-transporte
...
BEGIN
    ...
    -- 2. Security Check (Admin Context handled by service_role caller verification)
    -- The backend verify the user is admin before calling this via service_role.
...
ALTER FUNCTION public.fail_checkout_attempt_initialization(UUID, UUID, UUID, BOOLEAN, TIMESTAMPTZ, TEXT) OWNER TO postgres;

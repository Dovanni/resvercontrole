-- Allow livemode events in process_stripe_webhook_event
-- Reason: Post-payment reconciliation for enterprise activation.
...
BEGIN
    ...
    INSERT INTO public.payment_events (
        ...
        provider_event_created_at, processing_status, empresa_id, subscription_id
    )
    VALUES (
        'stripe', p_provider_event_id, p_event_type, p_payload_sha256, 
        p_event_created, 'processing', null, null
    )
    ...
    BEGIN
        v_internal_sub_id := (v_metadata->>'subscription_id')::UUID; -- Try subscription_id directly if internal_subscription_id is missing
    ...
    -- Validation of plan code if provided in canonical form
    IF (v_obs_plan_code IS NOT NULL AND v_obs_plan_code <> p_canonical_plan_code) THEN
        -- Allow if canonical is default and metadata has something else that we trust?
        -- No, let's stick to contract.
    END IF;
    ...
    CASE p_event_type
        ...
        WHEN 'customer.subscription.created', 'customer.subscription.updated' THEN
            ...
            UPDATE public.subscriptions SET 
                ...
                updated_at = now(),
                plan_id = (SELECT id FROM public.plans WHERE code = 'enterprise_monthly' LIMIT 1) -- Ensure plan is updated to enterprise
            WHERE id = v_internal_sub_id;
        ...
        WHEN 'invoice.paid' THEN
            UPDATE public.subscriptions SET 
                ...
                updated_at = now(),
                plan_id = (SELECT id FROM public.plans WHERE code = 'enterprise_monthly' LIMIT 1)
            WHERE id = v_internal_sub_id;

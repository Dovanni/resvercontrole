...
BEGIN
  ...
  -- Lucro = Receber + Frete Cliente - Custo - Juros ML - Frete Empresa
  v_lucro := v_loja + v_frete_cli - v_cost - v_juros - v_frete_emp;
...
-- Recalcular todos os lançamentos automáticos com a nova fórmula
DO $$
DECLARE r record;
BEGIN
...
-- Recalcular lançamentos manuais (sem sale_id) com a nova fórmula
UPDATE public.controle_vendas_diario
   SET lucro = loja + COALESCE(frete_cliente,0) - COALESCE(custo,0) - COALESCE(juros_ml,0) - COALESCE(frete_empresa,0),
       receber = loja

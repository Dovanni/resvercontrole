...
-- P.10-C6.SEO-IMPL-R2 — persistência de controles SEO por artigo (repository-only)
-- NÃO aplicar ao Supabase sem validação executável e autorização explícita.

begin;

alter table public.blog_posts
  add column seo_allow_indexing boolean not null default true,
  add column seo_allow_following boolean not null default true,
  add column seo_include_in_sitemap boolean not null default true;

alter table public.blog_posts
  add constraint blog_posts_seo_sitemap_requires_indexing_check
  check (seo_allow_indexing or not seo_include_in_sitemap);

-- SEO é conteúdo editorial material: qualquer alteração nos três flags cria
-- nova revisão e invalida a aprovação da revisão anterior, preservando o
-- contrato de quatro-olhos existente.
...
-- A assinatura muda, portanto a função anterior precisa ser removida antes da
-- criação da versão que recebe os três controles SEO.
drop function public.blog_save_draft_transaction(
  text, uuid, integer, text, text, text, jsonb, uuid, uuid, uuid[],
  text, text, text, text, text, integer
);

create function public.blog_save_draft_transaction(
  p_operation text,
  p_post_id uuid,
...
declare
  ...
  _seo_allow_indexing boolean := coalesce(p_seo_allow_indexing, true);
  _seo_allow_following boolean := coalesce(p_seo_allow_following, true);
  _seo_include_in_sitemap boolean := case
    when coalesce(p_seo_allow_indexing, true) then coalesce(p_seo_include_in_sitemap, true)
    else false
  end;
  _result_id uuid;
...
begin
  ...
  if p_operation = 'create' then
    ...
    insert into public.blog_posts (
      ...
      seo_allow_indexing, seo_allow_following, seo_include_in_sitemap
    ) values (
      ...
      _seo_allow_indexing, _seo_allow_following, _seo_include_in_sitemap
    )
  ...
  set
    ...
    seo_allow_indexing = _seo_allow_indexing,
    seo_allow_following = _seo_allow_following,
    seo_include_in_sitemap = _seo_include_in_sitemap
...
revoke all on function public.blog_save_draft_transaction(
  text, uuid, integer, text, text, text, jsonb, uuid, uuid, uuid[],
  text, text, text, text, text, integer, boolean, boolean, boolean
) from public;

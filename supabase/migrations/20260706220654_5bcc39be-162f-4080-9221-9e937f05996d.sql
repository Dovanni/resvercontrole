...
BEGIN
  ...
  -- Custo LÍQUIDO por item: usa a média ponderada do custo líquido histórico das
  -- compras do produto (preço unitário × (1 - desconto/subtotal da compra)).
  -- Fallback para sale_items.unit_cost quando o produto ainda não tem compras.
  WITH avg_net AS (
  ...
  -- Lucro = Receber - Custo líquido - Juros ML - Frete empresa
  -- (Frete cliente já compõe o total/receber e não é subtraído novamente)
...
-- Ressincroniza vendas entregues com o novo custo líquido
DO $$
DECLARE r record;
BEGIN

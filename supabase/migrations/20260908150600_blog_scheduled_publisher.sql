...
alter table public.blog_workflow_events
  add constraint blog_workflow_events_actor_source_check
  check (actor_source in ('human', 'scheduler', 'system'));
...
create table blog_private.blog_scheduled_publication_attempts (
  ...
  error_code text,
  detail text,
  constraint blog_scheduled_publication_attempts_outcome_check
    check (outcome in ('published', 'skipped_no_approval', 'skipped_requirements', 'failed'))
);

create index blog_scheduled_publication_attempts_post_attempted_idx
  on blog_private.blog_scheduled_publication_attempts (post_id, attempted_at desc);
...
as $$
declare
  _scheduler_context boolean := current_setting('blog.scheduler_context', true) = 'scheduled_publisher';
  _actor_user_id uuid;
  _actor_source text;
begin
  if _scheduler_context then
    _actor_user_id := null;
    _actor_source := 'scheduler';
  else
    _actor_user_id := coalesce(auth.uid(), new.updated_by, new.created_by);
    _actor_source := 'human';
  end if;

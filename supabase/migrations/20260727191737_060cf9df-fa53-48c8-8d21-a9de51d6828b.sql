-- 1) Revoga qualquer acesso pré-existente
REVOKE ALL ON FUNCTION public.rpc_editar_compra_pendente(
  uuid, timestamptz, uuid, date, text, text, integer, date, numeric, numeric, text, jsonb
) FROM PUBLIC, anon, authenticated, service_role, sandbox_exec;

-- 2) Substitui a função com search_path endurecido, optimistic-lock obrigatório
--    e referências qualificadas em todas as tabelas.
...
-- 3) Grants finais: somente authenticated
REVOKE ALL ON FUNCTION public.rpc_editar_compra_pendente(
  uuid, timestamptz, uuid, date, text, text, integer, date, numeric, numeric, text, jsonb
) FROM PUBLIC, anon, service_role, sandbox_exec;

GRANT EXECUTE ON FUNCTION public.rpc_editar_compra_pendente(
  uuid, timestamptz, uuid, date, text, text, integer, date, numeric, numeric, text, jsonb
) TO authenticated;
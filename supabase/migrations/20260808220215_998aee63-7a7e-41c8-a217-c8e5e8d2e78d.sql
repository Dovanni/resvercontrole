...
CREATE OR REPLACE FUNCTION public.get_company_subscription_context(p_empresa_id uuid, p_user_id uuid DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    -- Priorizar p_user_id (passado pelo servidor) ou auth.uid() (passado pelo cliente)
    ...
    -- Validar membership active da empresa solicitada
    SELECT EXISTS (
...
ALTER FUNCTION public.get_company_subscription_context(uuid, uuid) OWNER TO postgres;

REVOKE ALL ON FUNCTION public.get_company_subscription_context(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_company_subscription_context(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_company_subscription_context(uuid, uuid) TO service_role;

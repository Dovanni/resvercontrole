...
BEGIN
  ...
  -- total = subtotal - discount + frete_cliente - juros  =>  frete_cliente = total - (subtotal - discount) + juros
  v_frete_cli := GREATEST(s.total - (v_subtotal - COALESCE(s.discount,0)) + v_juros, 0);
  v_date := s.sold_at::date;
  -- Lucro = Receber - Custo - Frete empresa (juros e frete cliente já compõem o total)
  v_lucro := v_loja - v_cost - v_frete_emp;
...
-- Resync existing rows so the fix reflects historical vendas entregues
DO $$
DECLARE r record;
BEGIN

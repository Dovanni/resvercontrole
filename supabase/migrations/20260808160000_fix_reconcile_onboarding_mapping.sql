-- Migration: fix_reconcile_onboarding_relation_mapping
-- Authorizes structural correction of public.reconcile_and_finalize_onboarding()
-- Replaces public.purchases with public.compras
...
BEGIN
    ...
    -- CORREÇÃO: Substituído public.purchases por public.compras
...
-- Revogar acessos públicos e anon
REVOKE ALL ON FUNCTION public.reconcile_and_finalize_onboarding() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.reconcile_and_finalize_onboarding() FROM anon;

-- Garantir acesso a usuários autenticados e service_role

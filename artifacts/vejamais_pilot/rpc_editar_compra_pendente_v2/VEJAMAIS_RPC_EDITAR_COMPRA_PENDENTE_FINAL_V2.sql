-- VEJAMAIS — Export read-only de public.rpc_editar_compra_pendente
-- Gerado em: 2026-07-27T19:35:29Z
-- Fonte: pg_get_functiondef(oid=26638)

-- owner: postgres | return: jsonb | security_definer: true | config: search_path=pg_catalog, public, pg_temp

-- === DEFINITION (pg_get_functiondef, byte-exact) ===
...
BEGIN
  ...
  -- V2: separa lock (SELECT ... FOR UPDATE em CTE MATERIALIZED, sem agregação)
  --     da agregação de IDs (array_agg somente na consulta externa),
  --     corrigindo SQLSTATE 0A000 ("FOR UPDATE is not allowed with aggregate functions").
...
-- === GRANTS ===
-- grantee=sandbox_exec privilege=EXECUTE grantable=NO

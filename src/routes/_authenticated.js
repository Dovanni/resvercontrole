...
function AuthedLayout() {
    ...
    useEffect(() => {
        if (!user?.id)
            return;
        supabase.from("company_settings").select("theme").eq("user_id", user.id).maybeSingle().then(({ data }) => {
    ...
    if (loading || (session && contextLoading)) {
        return (<div className="min-h-screen flex items-center justify-center text-muted-foreground text-sm">
        Carregando…
      </div>);
    }
    if (!session) {

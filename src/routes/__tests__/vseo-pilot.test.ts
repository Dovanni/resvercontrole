...
describe('VSEO Pilot Lab Mocks', () => {
  ...
  it('all article slugs must be unique', () => {
    const slugs = MOCK_ARTICLES.map((a: MockArticle) => a.slug);
    const uniqueSlugs = new Set(slugs);
    expect(uniqueSlugs.size).toBe(slugs.length);
  });

  it('all meta titles must be present and unique', () => {
    const titles = MOCK_ARTICLES.map((a: MockArticle) => a.metaTitle);
    titles.forEach((t: string) => expect(t.length).toBeGreaterThan(0));
    const uniqueTitles = new Set(titles);
    expect(uniqueTitles.size).toBe(titles.length);
  });

  it('all meta descriptions must be present and unique', () => {
    const descs = MOCK_ARTICLES.map((a: MockArticle) => a.metaDescription);
    descs.forEach((d: string) => expect(d.length).toBeGreaterThan(0));
    const uniqueDescs = new Set(descs);
    expect(uniqueDescs.size).toBe(descs.length);
  });

  it('all articles should have a focus keyword', () => {
    MOCK_ARTICLES.forEach((article: MockArticle) => {

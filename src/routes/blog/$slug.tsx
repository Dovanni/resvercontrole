...
import { isSafeBlogLinkHref } from "@/features/blog/blog-content";
...
import type { BlogArticleParagraph } from "@/features/blog/types";
...
function BlogArticlePage() {
  ...
  return <BlogShell><main><article>
    ...
    <div className="border-y bg-card/35"><div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 md:px-6 lg:grid-cols-[minmax(0,1fr)_260px]"><div className="mx-auto w-full max-w-3xl space-y-12 lg:mx-0">{article.sections.map((section,index) => <section key={`${section.heading}-${index}`} id={`secao-${index+1}`}>{section.headingLevel === 3 ? <h3 className="font-display text-xl font-semibold md:text-2xl">{section.heading}</h3> : <h2 className="font-display text-2xl font-semibold md:text-3xl">{section.heading}</h2>}<div className="mt-5 space-y-5 text-base leading-8 text-muted-foreground md:text-lg">{section.paragraphs.map((paragraph, paragraphIndex) => <p key={`${section.heading}-${paragraphIndex}`}><StructuredParagraph paragraph={paragraph} /></p>)}</div></section>)}</div><aside className="hidden rounded-2xl border bg-background p-5 lg:block"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Neste artigo</p><ol className="mt-4 space-y-3 text-sm text-muted-foreground">{article.sections.map((s,i)=><li key={`${s.heading}-${i}`} className={s.headingLevel === 3 ? "pl-4" : undefined}><a href={`#secao-${i+1}`}>{s.heading}</a></li>)}</ol></aside></div></div>
...
function StructuredParagraph({ paragraph }: { paragraph: BlogArticleParagraph }) {
  if (typeof paragraph === "string") return <>{paragraph}</>;
  return <>{paragraph.content.map((item, index) => {
    if (item.type === "text") return <React.Fragment key={`text-${index}`}>{item.text}</React.Fragment>;
    if (!isSafeBlogLinkHref(item.href)) return <React.Fragment key={`unsafe-${index}`}>{item.text}</React.Fragment>;
    const external = item.href.startsWith("https://");
    return <a key={`link-${index}`} href={item.href} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined} className="font-medium text-primary underline decoration-primary/35 underline-offset-4 hover:decoration-primary">{item.text}</a>;
  })}</>;
}

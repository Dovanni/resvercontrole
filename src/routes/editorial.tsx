...
import {
  ...
  revisionLabel,
  workflowStatusLabel,
} from "@/features/blog/editorial-localization";
import {
...
function EditorialRoute() {
  ...
  if (authLoading || state.kind === "loading") {
    return <EditorialMessage title="Validando acesso editorial" description="Conferindo sua sessão exclusiva do Blog, vínculo editorial e permissões de leitura." />;
  }
  if (!user) {
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b bg-background/95">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 md:px-6">
          ...
          <div className="flex items-center gap-3"><span className="hidden rounded-full border px-3 py-1 text-xs font-semibold text-muted-foreground sm:inline-flex">{editorialRoleLabel(model.member.role)}</span><Link to="/editorial/editor" className="text-sm font-semibold text-primary hover:underline">Abrir editor</Link><Link to="/blog" className="text-sm font-semibold text-primary hover:underline">Ver Blog</Link></div>
        </div>
      </header>

      <main className="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-14">
        <section className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Operação editorial</p><h1 className="mt-3 max-w-3xl font-display text-4xl font-bold tracking-tight text-petrol md:text-5xl">Painel editorial administrativo</h1><p className="mt-4 max-w-2xl leading-7 text-muted-foreground">Acompanhe artigos, catálogos, revisões e eventos do fluxo editorial protegidos por RLS. Este painel é dedicado à consulta; criação, edição e publicação ficam no Editor Editorial V2.</p></div>
          <div className="rounded-2xl border bg-card p-4 text-sm shadow-sm"><div className="flex items-center gap-2 font-semibold"><CheckCircle2 className="size-4 text-primary" /> Modelo de consulta ativo</div><div className="mt-2 flex items-center gap-2 text-muted-foreground"><BookOpen className="size-4" /> Escrita disponível no editor</div></div>
        </section>
        ...
        <section className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricCard icon={<Tags className="size-5" />} label="Marcadores" value={model.catalog.tags.length} />
        ...
        <section className="mt-8 rounded-3xl border bg-card/60 p-6 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4"><div><div className="flex items-center gap-2"><Filter className="size-5 text-primary" /><h2 className="font-display text-2xl font-semibold">Artigos</h2></div><p className="mt-1 text-sm text-muted-foreground">Filtre pelo estado atual do fluxo editorial.</p></div><div className="flex flex-wrap gap-2">{STATUS_OPTIONS.map((option) => <button key={option.value} type="button" onClick={() => setStatusFilter(option.value)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${statusFilter === option.value ? "bg-primary text-primary-foreground" : "bg-background text-muted-foreground"}`}>{option.label}{option.value !== "all" ? ` · ${counts[option.value]}` : ""}</button>)}</div></div>

          <div className="mt-6 overflow-hidden rounded-2xl border bg-background">
            {filteredPosts.length === 0 ? <div className="p-8 text-center text-sm text-muted-foreground">Nenhum artigo encontrado neste estado.</div> : filteredPosts.map((post) => <button key={post.id} type="button" onClick={() => setSelectedPostId(post.id)} className={`grid w-full gap-2 border-b p-4 text-left last:border-b-0 md:grid-cols-[1fr_auto_auto] md:items-center ${selectedPostId === post.id ? "bg-muted/60" : "hover:bg-muted/30"}`}><div><p className="font-semibold text-petrol">{post.title}</p><p className="mt-1 text-xs text-muted-foreground">/{post.slug} · {revisionLabel(post.revisionNumber)} · {post.category ?? "Sem categoria"}</p></div><StatusBadge status={post.status} /><span className="text-xs text-muted-foreground">{formatDate(post.updatedAt)}</span></button>)}
          </div>
        </section>

        <section className="mt-8 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-3xl border bg-card/60 p-6 shadow-sm">
            <div className="flex items-center gap-2"><History className="size-5 text-primary" /><h2 className="font-display text-2xl font-semibold">Detalhes e linha do tempo</h2></div>
            {detailLoading ? <p className="mt-6 text-sm text-muted-foreground">Carregando histórico editorial…</p> : !detail ? <p className="mt-6 text-sm text-muted-foreground">Selecione um artigo para consultar revisões, avaliações e fluxo editorial.</p> : <div className="mt-6 space-y-6"><div className="rounded-2xl border bg-background p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold text-petrol">{detail.post.title}</p><p className="mt-1 text-xs text-muted-foreground">Revisão atual {detail.post.revisionNumber} · {detail.post.author ?? "Autor não vinculado"}</p></div><StatusBadge status={detail.post.status} /></div></div><TimelineSection title="Fluxo editorial" icon={<FileClock className="size-4" />} items={detail.workflow.map((event) => ({ id: event.id, title: `${workflowStatusLabel(event.fromStatus)} → ${workflowStatusLabel(event.toStatus)}`, meta: `${formatDate(event.createdAt)}${event.actorUserId ? ` · ${shortId(event.actorUserId)}` : ""}`, note: event.note }))} empty="Nenhum evento do fluxo editorial." /><TimelineSection title="Avaliações" icon={<ShieldCheck className="size-4" />} items={detail.reviews.map((review) => ({ id: review.id, title: `${review.decision === "approved" ? "Aprovado" : "Ajustes solicitados"} · revisão ${review.revisionNumber}`, meta: `${formatDate(review.createdAt)} · ${shortId(review.reviewerUserId)}`, note: review.notes }))} empty="Nenhuma decisão de avaliação." /><TimelineSection title="Revisões" icon={<History className="size-4" />} items={detail.revisions.map((revision) => ({ id: revision.id, title: `Revisão ${revision.revisionNumber}`, meta: `${formatDate(revision.createdAt)}${revision.createdBy ? ` · ${shortId(revision.createdBy)}` : ""}`, note: revision.reason }))} empty="Nenhum registro de estado da revisão." /></div>}
          </div>

          <aside className="space-y-6">
            <div className="rounded-3xl border bg-card/60 p-6 shadow-sm"><div className="flex items-center gap-2"><UserRound className="size-5 text-primary" /><h2 className="font-display text-2xl font-semibold">Seu acesso</h2></div><dl className="mt-5 space-y-3 text-sm"><InfoRow label="Papel" value={editorialRoleLabel(model.member.role)} /><InfoRow label="Vínculo editorial" value={model.member.active ? "Ativo" : "Inativo"} /><InfoRow label="Perfil de autor" value={model.member.authorId ? "Vinculado" : "Não vinculado"} /><InfoRow label="Registro do estado" value={formatDate(model.loadedAt)} /></dl></div>
            <div className="rounded-3xl border bg-card/60 p-6 shadow-sm"><div className="flex items-center gap-2"><CalendarClock className="size-5 text-primary" /><h2 className="font-display text-2xl font-semibold">Catálogo</h2></div><div className="mt-5 space-y-4 text-sm"><CatalogList label="Categorias" items={model.catalog.categories.map((item) => item.name)} /><CatalogList label="Autores" items={model.catalog.authors.map((item) => item.displayName)} /><CatalogList label="Marcadores" items={model.catalog.tags.map((item) => item.name)} /></div></div>
...
function StatusBadge({ status }: { status: BlogPostStatus }) { return <span className="w-fit rounded-full border px-2.5 py-1 text-xs font-semibold text-muted-foreground">{blogPostStatusLabel(status)}</span>; }
function InfoRow({ label, value }: { label: string; value: string }) { return <div><dt className="text-muted-foreground">{label}</dt><dd className="mt-1 font-semibold">{value}</dd></div>; }

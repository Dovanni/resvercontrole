      ...
      <header className="border-b bg-background/95">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 md:px-6">
          <div className="flex items-center gap-3">
            ...
            <div><p className="font-display text-lg font-semibold text-petrol">VEJAMAIS ERP Editorial</p><p className="text-xs text-muted-foreground">Read model administrativo · modo somente leitura</p></div>
          </div>
          <div className="flex items-center gap-3"><span className="hidden rounded-full border px-3 py-1 text-xs font-semibold capitalize text-muted-foreground sm:inline-flex">{model.member.role}</span><Link to="/blog" className="text-sm font-semibold text-primary hover:underline">Ver preview</Link></div>
        </div>
      </header>

      <main className="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-14">
        <section className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Fase 3-S · Repository-only</p><h1 className="mt-3 max-w-3xl font-display text-4xl font-bold tracking-tight text-petrol md:text-5xl">Painel editorial administrativo em leitura real</h1><p className="mt-4 max-w-2xl leading-7 text-muted-foreground">Artigos, catálogos, revisões e eventos de workflow são lidos das tabelas editoriais protegidas por RLS. Nenhum comando desta tela executa escrita.</p></div>
          <div className="rounded-2xl border bg-card p-4 text-sm shadow-sm"><div className="flex items-center gap-2 font-semibold"><CheckCircle2 className="size-4 text-primary" /> Read model ativo</div><div className="mt-2 flex items-center gap-2 text-muted-foreground"><LockKeyhole className="size-4" /> Escrita continua bloqueada</div></div>
        </section>
        ...
        <section className="mt-8 rounded-3xl border bg-card/60 p-6 shadow-sm">
          ...
          <div className="mt-6 overflow-hidden rounded-2xl border bg-background">
            {filteredPosts.length === 0 ? <div className="p-8 text-center text-sm text-muted-foreground">Nenhum artigo encontrado neste status. Os três drafts homologados continuam locais e ainda não foram importados.</div> : filteredPosts.map((post) => <button key={post.id} type="button" onClick={() => setSelectedPostId(post.id)} className={`grid w-full gap-2 border-b p-4 text-left last:border-b-0 md:grid-cols-[1fr_auto_auto] md:items-center ${selectedPostId === post.id ? "bg-muted/60" : "hover:bg-muted/30"}`}><div><p className="font-semibold text-petrol">{post.title}</p><p className="mt-1 text-xs text-muted-foreground">/{post.slug} · rev. {post.revisionNumber} · {post.category ?? "Sem categoria"}</p></div><StatusBadge status={post.status} /><span className="text-xs text-muted-foreground">{formatDate(post.updatedAt)}</span></button>)}
          </div>
        </section>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <aside className="space-y-6">
            ...
            <div className="flex items-start gap-2 rounded-2xl border border-dashed p-4 text-xs leading-5 text-muted-foreground"><Clock3 className="mt-0.5 size-4 shrink-0" />A UI de escrita permanece isolada e com executor fail-closed. Esta tela contém somente consultas.</div>
          </aside>
        </section>
      </main>
    </div>
